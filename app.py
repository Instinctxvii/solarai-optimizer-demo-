...
view_mode = st.radio("Select View:", ["Today Only", "14-Day Forecast"], horizontal=True)

if view_mode == "Today Only":
    df_view = df[df['Time'].dt.date == datetime.now().date()]
else:
    df_view = df
...
fig = px.line(
    ...
    x='Time',
    y='Solar Yield (W/m²)',
    title=f"GHI — {loc['name']} ({'Today Only' if view_mode == 'Today Only' else '14-Day Forecast'})",
...
**Example:** At **12:15 PM**, yield = **950 W/m²** → **perfect time to turn on geyser!**
""")
